...
class Message:
    ...
    def read_headers(self, header_data):
        ...
        while i < len(header_data):
            ...
            ID = struct.unpack(">B", bytes([header_data[i]]))[0]
            ID_type = ID & 0xc0
            if ID_type == 0x00:
                # text
                length = struct.unpack(">H", bytes(header_data[i+1:i+3]))[0] - 3
    ...
    def encode(self):
        ...
        ret = struct.pack(*args)
        for header_data in [h.data for h in self.header_data]:
            ret += header_data
